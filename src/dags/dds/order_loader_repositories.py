...
class OrderStgRepository:
    def load_raw_orders(self, conn: Connection, last_loaded_record_id: int) -> List[OrderStgObj]:
        with conn.cursor(row_factory=class_row(OrderStgObj)) as cur:
            cur.execute(
                """
                    SELECT
                        id,
...
class OrderLoader:
    ...
    def parse_order(self, order_raw: OrderStgObj, restaurant_id: int, timestamp_id: int, user_id: int) -> OrderDdsObj:
